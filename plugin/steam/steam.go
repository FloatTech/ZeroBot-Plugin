...
var (
	engine = control.Register("steam", &ctrl.Options[*zero.Ctx]{
		...
		Brief:            "视奸steam好友",
		Help: "- 创建steam用户绑定 xxxxxxx (可输入需要绑定的 steamid )\n" +
			...
			"- 查看当前steam链接密钥 (查询已经绑定的密钥)\n" +
			"- 拉取steam绑定用户状态 (使用插件定时任务开始)\n-----------------------\n" +
			"Tips: steamID在用户资料页的链接上面, 形如7656119820673xxxx, 需要管理员先开启查询\n需要先进行密钥初始化, 绑定用户之后使用定时任务开启查询即可自动播报",
...
func init() {
	// 创建绑定流程
	engine.OnRegex(`^创建steam用户绑定\s*(\d+)$`, zero.OnlyGroup, getDB).SetBlock(true).Handle(func(ctx *zero.Ctx) {
	...
	// 删除绑定流程
	engine.OnRegex(`^删除steam用户绑定\s*(\d+)$`, zero.OnlyGroup, getDB).SetBlock(true).Handle(func(ctx *zero.Ctx) {
		steamID := ctx.State["regex_matched"].([]string)[1]
		groupID := strconv.FormatInt(ctx.Event.GroupID, 10)
