...
// 获取或创建账户
func GetOrCreateAccount(uid int64) *BankAccount {
...
// 保存账户信息
func SaveAccount(account *BankAccount) error {
...
func checkAndUpdateInterest() {
	...
	if dailyInterest.LastDate != today {
		// 每天随机生成活期利率（1% - 3%）
		dailyInterest.CurrentRate = 0.01 + rand.Float64()*0.02
...
// 检查是否有“未还清且未逾期”的贷款（私有函数）
func hasActiveLoan(account *BankAccount) bool {
	for _, loan := range account.Loans {
		totalRepay := calculateLoanRepay(loan.Amount, loan.Rate, loan.TermDays)
		if loan.RepaidAmount < totalRepay && !loan.IsOverdue {
			return true
		}
	}
	return false
...
// 专门处理逾期贷款的扣款逻辑
func deductOverdueLoan(account *BankAccount, loanIndex int, now time.Time) {
...
// 清理账户中已完成的贷款记录
func cleanupCompletedLoans(account *BankAccount) {
	filteredLoans := []Loan{}
	for _, loan := range account.Loans {
		// 保留未还清的贷款（已还金额 < 应还总额）
		totalRepay := calculateLoanRepay(loan.Amount, loan.Rate, loan.TermDays)
...
func hasUnpayableLoan(account *BankAccount) bool {
    // 钱包余额判断
...
// 执行信仰之跃重生
func applyRebirth(account *BankAccount) {
    // 1. 清零所有贷款
    account.Loans = []Loan{}
...
// 获取重生buff的有效层数
func getRebirthStacks(account *BankAccount) int {
	now := time.Now()
	stacks := 0
...
func addRebirthBuff(account *BankAccount) {
	now := time.Now()
	// 先清理过期buff，避免无效数据干扰
...
// 清理账户中所有过期的重生buff
func cleanupExpiredBuffs(account *BankAccount) {
	now := time.Now()
	validBuffs := []Buff{}
	for _, buff := range account.Buffs {
...
// 检查账户是否有有效的重生buff
func hasRebirthBuff(account *BankAccount) bool {
	return getRebirthStacks(account) > 0
}

// 计算最早过期的重生buff剩余时间
func getEarliestRebirthBuffExpiry(account *BankAccount) (time.Duration, bool) {
    now := time.Now()
...
// 强制还款（完全复刻钱包→活期→定期的扣款逻辑）
func forceRepayLoan(account *BankAccount, loan *Loan, now time.Time) (bool, string) {
    // 计算需扣除的总金额
    totalRepay := calculateLoanRepay(loan.Amount, loan.Rate, loan.TermDays)
    needDeduct := totalRepay - float64(loan.RepaidAmount)
    if needDeduct <= 0 {
        return true, "该贷款已还清"
    }
    ...
    if walletBalance > 0 {
        ...
        loan.RepaidAmount += float64(int(math.Round(deduct)))
        needDeduct -= deduct
        fmt.Printf("[强制扣款] 从钱包扣除: %.2f，剩余: %.2f\n", deduct, needDeduct)
    }
    ...
    if needDeduct <= 0 {
        account.LastUpdate = now
        _ = SaveAccount(account)
        fmt.Printf("[强制扣款] 用户%d的贷款已还清\n", account.UserID)
        cleanupCompletedLoans(account)
        return true, "从钱包扣款后已全额还清"
    }
    ...
    if account.CurrentBalance > 0 {
        ...
        loan.RepaidAmount += deduct
        needDeduct -= deduct
        fmt.Printf("[强制扣款] 从活期扣除: %.2f，剩余: %.2f\n", deduct, needDeduct)
    }
    ...
    if needDeduct <= 0 {
        ...
        fmt.Printf("[强制扣款] 用户%d的贷款已还清\n", account.UserID)
        cleanupCompletedLoans(account)
        return true, "从活期扣款后已全额还清"
    }

    // 3. 从定期存款扣款（含70%手续费）
    feeRate := 0.5 // 固定手续费率70%
    for len(account.FixedDeposits) > 0 && needDeduct > 0 {
        fd := &account.FixedDeposits[0]
        // 计算扣除手续费后的可用金额
        available := float64(fd.Amount) * (1 - feeRate)
        ...
        // 计算实际消耗的定期本金（含手续费部分）
        usedPrincipal := int(math.Ceil(deduct / (1 - feeRate)))
        fmt.Printf("[强制扣款] 从定期扣除: %.2f（本金消耗: %d，手续费率: 70%%）\n", deduct, usedPrincipal)
        
        // 更新定期存款记录
        if usedPrincipal >= fd.Amount {
            // 全额取出，移除该定期记录
            account.FixedDeposits = account.FixedDeposits[1:]
        } else {
            // 部分取出，更新剩余金额
            fd.Amount -= usedPrincipal
        }
    }
    ...
    if needDeduct <= 0 {
        fmt.Printf("[强制扣款] 用户%d的贷款已还清\n", account.UserID)
        cleanupCompletedLoans(account)
        return true, "从定期扣款后已全额还清"
    } else {
        return false, fmt.Sprintf("扣款后仍有%.2f未还清", needDeduct)
    }
}