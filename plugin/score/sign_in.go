...
var levelArray = [...]int{0, 1, 2, 6, 12, 16, 24, 32, 48, 52, 64, 72, 88, 96, 112, 120, 144, 168, 172, 192, 212, 224, 240}
	...
	engine.OnFullMatch("签到", zero.OnlyGroup).Limit(ctxext.LimitByGroup).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			...
			if si.Count >= signinMax && siUpdateTimeStr == today {
				ctx.SendChain(message.Reply(ctx.Event.MessageID), message.Text("今天你签到过了~"))
			...
			canvas.DrawString("LV-"+strconv.FormatInt(int64(level), 10), float64(back.Bounds().Size().X)*0.1, float64(back.Bounds().Size().Y)*1.5)
