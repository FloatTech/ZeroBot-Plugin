...
func init() { // 插件主体
	engine := control.Register(serviceName, &ctrl.Options[*zero.Ctx]{
		...
		Help: "AI画图\n" +
			"基于百度文心的免费AI画图插件,\n因为是免费的\n" +
			"账号申请链接：https://wenxin.baidu.com/moduleApi/key\n" +
		...
		single.WithPostFn[int64](func(ctx *zero.Ctx) {
			_ = ctx.GetMatcher().SetBlock(true)
			ctx.Send(
	...
	// 画图
	regex := `(` + strings.Join(dtype[:], "|") + `|.+)`
	engine.OnRegex(`画几张(.*[^的$])的`+regex+`(方图|长图|横图)$`, zero.OnlyToMe, getdb).SetBlock(true).
	...
	engine.OnRegex(`^为(群)?(自己|本群|\d+)设置AI画图key\s(.*[^\s$])\s(.+)$`, getdb).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			...
			case user == "本群": // 用于本群
				gid := ctx.Event.GroupID
				if gid == 0 {
					ctx.SendChain(message.Text(serviceErr, "ERROR:请指定群聊，或者使用指令；\n为群xxx设置AI画图key xxx xxx"))
					return
				}
				dbID = gid
...
func (sql *keydb) insert(gid int64, akey, skey string) error {
	...
	if err != nil {
		return sql.db.Insert("groupinfo", &apikey{
			ID:        gid,
			APIKey:    akey,
			SecretKey: skey,
			MaxLimit:  500,
